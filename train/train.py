...
class SVDFLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        # Reshape to [batch, input_time_size, input_frequency_size]
        x = tf.reshape(
            inputs,
            [batch_size, self.input_time_size, self.input_freq_size])

        # Apply frequency filters via Conv1D (stride = input_freq_size, VALID).
        # weights_frequency: [input_freq_size, 1, num_filters]
        w_freq = tf.expand_dims(self.weights_frequency, 1)
...
class SVDFStreamingLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, num_new_frames=None):
        ...
        # Reshape new input to [batch, num_new_frames, freq_size]
        x = tf.reshape(inputs,
                       [batch_size, num_new_frames,
                        self._svdf.input_freq_size])

        # Apply frequency filters → [batch, num_new_frames, num_filters]
        w_freq   = tf.expand_dims(self._svdf.weights_frequency, 1)
        new_acts = tf.nn.conv1d(x, w_freq, stride=1, padding='VALID')
